...
| `RegisterForm` Component  | Register with Google/Apple (social login) | Buttons for 'Register with Google' and 'Register with Apple' are visible and functional on the registration page. | OAuth logic and UI components exist, but buttons are NOT rendered in the registration form. | Not Implemented | Users cannot register with Google/Apple from registration page. | None | Add OAuthButtons to registration form; E2E/integration test for social registration. |
...
| LoginForm Component | Sign in with Google/Apple (social login) | Buttons for 'Sign in with Google' and 'Sign in with Apple' are visible and functional on the login page. | OAuth logic and UI components exist, but buttons are NOT rendered in the login form. | Not Implemented | Users cannot sign in with Google/Apple from login page. | None | Add OAuthButtons to login form; E2E/integration test for social login. |
...
| Profile/Settings UI | User Profile Verification UI | Users can request, view, or manage verification of their individual profile (e.g., verified badge, request verification button). | No UI for user profile verification is present in profile or settings. Only company/domain verification UI exists. | Not Implemented | No way for users to request or view profile verification. | None | Add user profile verification UI; E2E/integration test for verification request and status. |
...
| Admin/Settings/Profile UI | Role/Permission Management UI | Admins/users can view and manage roles/permissions through the UI. | User/role listing, assignment, and removal UI is now implemented in the admin panel. Permissions viewer and integration/E2E tests are still missing. | Partially Implemented | UI for listing, assigning, and removing roles is present. Permissions viewer and tests still needed. | `RoleManagementPanel.test.tsx` | E2E test: Add for role/permission management UI (integration tests implemented). |
