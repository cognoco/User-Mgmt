        ...
        if (fs.existsSync(srcTestDir) && fs.statSync(srcTestDir).isDirectory()) {
          fs.readdirSync(srcTestDir).forEach(testFile => {
            ...
            if (fs.statSync(srcTestPath).isFile()) {
              ...
              updateFileImports(styledTestPath, domain);
            }
          });
        }
...
/**
 * Updates import paths in a specific file
 */
function updateFileImports(filePath, domain) {
  try {
    let content = fs.readFileSync(filePath, 'utf8');
    let updated = false;
    
    // Update relative imports in test files (e.g., '../AuditLogViewer' -> '../../../styled/audit/AuditLogViewer')
    const relativeComponentImport = new RegExp(`from ['"]\\.\\./(\\w+)['"]`, 'g');
    
    if (relativeComponentImport.test(content)) {
      content = content.replace(relativeComponentImport, (match, componentName) => {
        return `from '../../../styled/${domain}/${componentName}'`;
      });
      updated = true;
    }
...
function updateImports(domain) {
  try {
    ...
    files.forEach(file => {
      // Skip the files in the new location to avoid circular updates
      if (file.includes(`/ui/styled/${domain}/`) || file.includes(`/ui/headless/${domain}/`)) {
        return;
      }
...
// MAIN
console.log('Starting migration for audit domain...');
