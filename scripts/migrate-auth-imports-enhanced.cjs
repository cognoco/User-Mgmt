/**
 * Enhanced Auth Import Path Migration Script
 * 
 * This script migrates auth-related import paths and usage to the new architecture pattern.
 * It not only updates import statements but also updates usage within components.
 * 
 * Usage:
 * node scripts/migrate-auth-imports-enhanced.cjs
 * 
 * Options:
...
const usagePatterns = [
  // Update direct useAuthStore() calls to useAuth()
  {
    pattern: /useAuthStore\(\)/g,
    replacement: 'useAuth()'
  },
  // Update useAuthStore selectors
  {
    pattern: /useAuthStore\(state\s*=>\s*state\.([a-zA-Z0-9]+)\)/g,
    replacement: (match, property) => {
      return `useAuth().${property}`;
    }
  },
  // Update mock setups
  {
    pattern: /vi\.mock\(['"]@\/lib\/stores\/auth\.store['"].*\{\s*useAuthStore:/gs,
    replacement: (match) => match.replace('useAuthStore:', 'useAuth:')
  },
  // Update mock implementations
  {
    pattern: /(useAuthStore\s+as\s+any)\.mockImplementation/g,
    replacement: '(useAuth as any).mockImplementation'
  },
  // Update function signatures that reference useAuthStore
  {
    pattern: /Partial<ReturnType<typeof useAuthStore>>/g,
    replacement: 'Partial<ReturnType<typeof useAuth>>'
  }
];
...
// Main execution
console.log(`Starting enhanced auth import path migration ${dryRun ? '(DRY RUN)' : ''}`);
