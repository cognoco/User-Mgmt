...
beforeEach(async () => {
  ...
  (mockAuthService.getCurrentUser as any).mockReturnValue(null);
...
describe('useAuth hook', () => {
  ...
  it('handles successful login', async () => {
    const user: User = { id: '1', email: 'a@test.com', mfaEnabled: true } as any;
    (mockAuthService.login as any).mockResolvedValue({ success: true, user, token: 'tok' });
    ...
    expect(mockAuthService.login).toHaveBeenCalledWith({ email: 'a@test.com', password: 'pass', rememberMe: true });
  ...
  it('registers a new user', async () => {
    ...
    (mockAuthService.register as any).mockResolvedValue({ success: true, user, token: 'tok2' });

    const { result } = renderHook(() => useAuth(), { wrapper });

    await act(async () => {
      await result.current.register({ email: 'new@test.com', password: 'p', confirmPassword: 'p', firstName: 'a', lastName: 'b', acceptTerms: true });
    });
  ...
  it('resets password and handles errors', async () => {
    (mockAuthService.resetPassword as any).mockResolvedValue({ success: true, message: 'sent' });
  ...
  it('handles MFA setup, verify and disable', async () => {
    (mockAuthService.setupMFA as any).mockResolvedValue({ success: true, secret: 's', qrCode: 'q', backupCodes: ['b'] });
    (mockAuthService.verifyMFA as any).mockResolvedValue({ success: true, backupCodes: ['b2'], token: 't' });
    (mockAuthService.disableMFA as any).mockResolvedValue({ success: true });

    const { result } = renderHook(() => useAuth(), { wrapper });
    await act(async () => { await result.current.setupMFA(); });
    expect(result.current.mfaSecret).toBe('s');
    await act(async () => { await result.current.verifyMFA('code'); });
    expect(result.current.token).toBe('t');
    await act(async () => { await result.current.disableMFA(); });
    expect(result.current.mfaEnabled).toBe(false);
  });
  ...
  it('refreshes token and tracks activity', async () => {
    ...
    const { result } = renderHook(() => useAuth(), { wrapper });
    await act(async () => { const ok = await result.current.refreshToken(); expect(ok).toBe(true); });

    (mockAuthService.refreshToken as any).mockRejectedValue(new Error('oops'));
    await act(async () => { const ok = await result.current.refreshToken(); expect(ok).toBe(false); });
  ...
  it('handles auth events', () => {
    ...
    (mockAuthService.onAuthEvent as any).mockImplementation((cb: any) => { eventHandler = cb; return () => 'unsub'; });
    const { result } = renderHook(() => useAuth(), { wrapper });
    const cb = vi.fn();
